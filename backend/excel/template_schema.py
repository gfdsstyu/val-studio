...
# ── 연도 열(명시 추정 + 페이드 실체화) ─────────────────────────────────────
# 12열(C..N) = 명시 최대 5년 + 페이드 최대 7년 여유(모델러스 5.4 실측 = 명시 5+페이드 5).
# 페이드 모델의 export 는 확장 열을 실체화해야 "수식 == 캐시값" 불변식이 성립한다
# (dcf_export 참조). 한도 초과는 export 가 **명시 에러**로 막는다 — 조용한 절단 금지.
YEAR_COLS = ["C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N"]
BASE_YEAR = 2024
...
# ── 모델 메타(개선 A/B 오버라이드 + 페이드 R1; 설정된 것만 기록) ────────────
# fade_years/fade_growth: export 는 페이드 열을 실체화(위 YEAR_COLS)하고 여기 파라미터를
# 기록한다 → import 가 뒤쪽 k열을 잘라 **파라메트릭 형태(3단 모델)** 로 복원한다.
# 이 두 셀이 없던 시절의 갭 실측: fade_years=3 왕복 시 주당가치 −23.1% 조용한 소실.
META = {
