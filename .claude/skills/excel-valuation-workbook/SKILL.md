...
## 공유 원장 (`_VS_FACTS` 시트) — **읽기 전용**

Val-Studio 애드인이 워크북에 남기는 **사실 원장**이다. 애드인끼리는 직접 통신할 수 없어
공유 매체가 워크북뿐인데, 그 쪽은 DART·스크리너·거시 커넥터(BYOK 키 필요)와 결정론
게이트를 갖고 있다 — 여기 적힌 값은 **이미 원천에서 확인된 것**이므로 다시 조사하지 말고
그대로 쓴다. 시트가 없으면 그냥 없는 것(정상 경로).

레이아웃: `facts_schema`·`owner` 키 → `── 사실 원장(append-only) ──` → 열이름 행 →
데이터. 열 = `순번|키|값|방법|원천|위치|조회일|신뢰도|승인상태|작성자`.
**열 위치를 가정하지 말고 열이름으로 읽는다**(열이 늘 수 있다. 모르는 열은 무시).

읽는 법 — **append-only 로그**다. 같은 `키`가 여러 번 나오면 오류가 아니라 이력이며,
**`순번`이 가장 큰 행이 현재 값**이다(옛 행은 증적으로 남는다). 예: `peer.145020.business`
= 그 종목의 사업 설명, `peer.145020.step2` = 사업유사성 판정과 사유.

⚠️ **쓰지 않는다.** 이 시트의 작성자는 Val-Studio 하나다(단일 작성자라 순번에 경합이
없다). 보호가 걸려 있고, 고쳐야 할 값이 있으면 사람에게 알린다. 워크북에 남길 내용은
`_VS_STATE`(가정 대장)와 `Claude Log` 로 쓴다.

⚠️ `승인상태=suggested` 는 **미승인 초안**이다 — `_VS_STATE` 가정 대장과 같은 규율로,
평가인 승인 없이 확정 가정으로 승격시키지 않는다.
