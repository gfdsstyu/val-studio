...
| 웹 UI | React+Vite, 평가인 21시트 + 감사인 9시트 (기법추천 위저드·산업 프로파일 대조) |
| 엑셀 통합 | 살아있는 수식 xlsx **4방향**(export⇄import⇄diff⇄정적감사) + Excel Task Pane 애드인 |
| 정본 템플릿 | **풀모델 26시트** `ValStudio_DCF_Template.xlsx` — 상향 배선(EBIT/FA/WC→DCF, WACC 빌드업) · 순환 0 · 서버 tie-out 왕복 |
...
## 엑셀과의 통합 — "엔진이 정본, 워크북은 표현"

실무는 엑셀을 떠나지 않는다. 그래서 웹으로 **대체**하지 않고 엑셀에 **접속**한다.
...
## MVP 목표 — "엑셀에서 시작해 엑셀에서 끝난다"

지금 조준하는 로컬 MVP의 완성 형태는 한 문장이다.

> **임의 상장사 하나를 정해, DART에서 재무제표를 엑셀로 직접 불러오고, 계정매핑·매출 세분화·
> 운전자본 추정을 AI 보조로 확정한 뒤, 엑셀을 떠나지 않고 평가를 마무리한다.**

핵심 설계 판단은 **웹 UI로 엑셀을 대체하지 않는다**는 것이다. 실무는 엑셀을 떠나지 않으므로,
워크북을 정본으로 두고 **세 창구가 같은 워크북에 접속**한다.

```mermaid
flowchart TB
  WB[("워크북 = 정본 · 상태<br/>ValStudio_DCF_Template.xlsx<br/>_VS_STATE · _DART_MAP")]
  CE["Claude for Excel<br/>(스킬)"]
  AD["Val-Studio 애드인<br/>(Task Pane)"]
  CC["Claude Code / CLI"]
  CE -- "판단 보조 — 계정매핑 제안·매출 쪼개기 근거<br/>회전일 리서치 · 결정론 스크립트" --> WB
  AD -- "네트워크 · 검증 — DART 조회→시트 기입<br/>getFileAsync 라이브 tie-out · 정적감사·diff" --> WB
  CC -- "배치 · 개발 — 인제스트·골든 회귀·룰 정련" --> WB
```

**왜 셋인가 — 실행 환경 제약이 다르다.** 스킬 샌드박스는 네트워크가 없어(설계상 붙여넣기 기반)
DART 직접 호출이 불가능하고, 애드인 Task Pane은 우리 웹앱이 뜨는 것이라 네트워크가 된다.
그래서 **네트워크가 필요한 일은 애드인이, 워크북 맥락을 읽는 대화형 판단은 Claude for Excel이**
맡는다. 둘은 직접 통신할 수 없으므로 **워크북 자체가 프로토콜**이 된다.
