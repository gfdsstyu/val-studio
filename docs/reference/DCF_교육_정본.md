---
topic: 참고 모델 DCF 교육 정본 — 평가접근법·FCFF/FCFE·재무제표 재분류·FCFF 산출·WACC·영구가치(도식 포함)
...
doc_type: knowledge
---
# 참고 모델 DCF 교육 정본 — 도식 중심 정리

> 출처: 참고 모델 「기업가치평가 연수 1기 — Business Valuation: 현금흐름할인법(DCF)」
...
| **β 추정치** | Bloomberg **2년 Weekly 또는 5년 Monthly 조정베타**(4대 법인). **감사인검토 Valuation Standard = 60개월 베타**. Capital IQ 관측·조정베타. 평가인 직접 Daily beta(자산평가사·Local 법인 일부). **비상장은 유사상장사 β 기반 대용치**(산업 유사성이 핵심 선정기준). Beta Mean Reversion = **Marshall Blume 조정** |
...
> 는 [[베타_Bloomberg_vs_KICPA]]·checks β/MRP 정합의 근거. 감사인검토 60개월은 [[감사인검토_WACC방법론]].
...
> → `wacc.kroll_size_premium` 와 동일 테이블 계보. [[감사인검토_WACC방법론]] Kroll deciles.

### 3.5 유사회사 산정
현재·향후 사업 성격 파악(구글링·증권사 리포트) → 전/후방 산업 검토 → KIND·CIQ 스크리닝
(유사성·완전성 확보). → 우리 [[리포트예시_클래시스]] §E 4-step·`peer_selection` 과 정합.
...
| WACC CAPM·Hamada·조정베타·size | `wacc.py`·[[베타_Bloomberg_vs_KICPA]]·[[감사인검토_WACC방법론]] |
