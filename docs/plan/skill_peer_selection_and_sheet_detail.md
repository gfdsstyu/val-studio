...
| **레퍼런스 정본** | `reference/wacc_할인율서식.md` §1~2 · `reference/M사_리포트예시_클래시스.md` §E · `backend/ingest/peer_selection.py` |
| **작성일** | 2026-07-19 |
...
Claude for Excel 실사용 피드백: 스킬 시트가 **모델링 기초 대비 얕고**, 특히 **유사회사 선정**이 M사 강의자료의 다단계 퍼널에 비해 빈약(3행 placeholder). 검토로 확인된 사실:
...
**할인율 서식 §1 (Step0~3)** + **M사 §E (4-step 실측)** + **peer_selection.py (실행 엔진)**:
...
**연결(절차적, 크로스시트 수식 아님)**: 확정 peer → WACC 무부채β·목표자본구조(D/Cap·E/Cap 평균). M사 11.유사회사 → 12.WACC 분리 구조 계승.
...
M사 모델 시트(모델링_실무 §2: `Assumption · DCF · EBIT · FA · WC · WACC · H_FS · BackData · 상각비계산 · 유사회사FS`) 대비 현 스켈레톤 갭을 매핑해 세분:
...
> 원칙: M사 **방법론·시트 골격 참조**, 시트명·레이아웃 **비복제**(자체 아키텍처). 살아있는 수식 우선.
...
- **M사 비복제**: 방법론·시트 골격만 차용, 셀 레이아웃 자체 정의.
